...
void check_simd_array_equal(simd_type<T, Width, ABI> value, std::array<T, Width> correct) {
  ...
  for (int i = 0; i < Width; i++) {
    ...
    } else if constexpr (std::is_same_v<T, std::complex<float>> or std::is_same_v<T, std::complex<double>>) {
      EXPECT_COMPLEX_NEAR(calculation[i], correct[i]);
    }
  }
}
...
TEST(NDA, OurSIMD) {
  // simd_type<int,5,abi_tag::Default> s;
  native_simd<long long> hop;
  ...
  // simd_cd2 x = {111, 234, 356, -478};
  // simd_cd2 y = {9, 10, -11, 12};
