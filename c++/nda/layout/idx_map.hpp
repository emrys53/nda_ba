...
namespace nda {
  ...
  class idx_map {
    ...
    [[nodiscard]] long capacity() const noexcept {
      //TODO ask if array is generated by size=capacity in basic_array then strides are have to be updated.
      // Also if we use capacity for any type of array. Our array will fail unless it is int/float/double/long
      // Suggestion: Create another array called nda:array_aligned with memory handle mallocator_aligned and do template specialization for it.
      // the layout might have to be also updated to work with aligned objects.
      size_t required_padding = padding.get_padding();
      if (padding.get_padding() == 0) { return size(); }
